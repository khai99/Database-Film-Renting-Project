...
<link rel = "shortcut icon" href = "img/fav.png" />
  ...
  <style>
   ...
   <script>
     function sameLan($value){
		 
		 if($value == 1){
		 document.getElementById("errorLanguage1").innerHTML="Invalid update : Same value found";
		 }else{
	...
	function dupli_langName($value){
		 
		 if($value == 1){
		 document.getElementById("dupliName").innerHTML="Invalid update : Duplicate value found";
		 }else{
			 
			 document.getElementById("dupliname").innerHTML="";
		 }
	...
	function error_Update($value){
		if($value == 1){
		 document.getElementById("dataError").innerHTML="Invalid update : Cannot be updated to database";
		 }else{
...
<body id="page-top">
<?php 
  if(isset($_POST['editLangu'])){
	  
	  $langID = $_POST['editLangu'];
	  
	  
  }else if(isset($_POST['update_language'])){
	  $langID = $_POST['update_language'];
   ...
   $sqlLanguageUP = "SELECT * FROM `language` WHERE language_id = '$langID'";
   $resultLanUP=mysqli_query($links,$sqlLanguageUP);
   $upLangRow=mysqli_fetch_assoc($resultLanUP);
...
<li class="nav-item">
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <!-- Page Heading -->
          <h1 class="h3 mb-4 text-gray-800">Update the language details</h1>
            ...
            <div class="col-lg-6">
             ...
             <div class="card shadow mb-4">
                <div class="card-header py-3">
                  ...
                  <h6 class="m-0 font-weight-bold text-primary">Update Language : <span style = "color:#e15d44;"><?php echo $upLangRow['language_id']; ?></span></h6>
                </div>
                <div class="card-body">
				<p id = "errorLanguage1" class = "errorMessage"></p>
					<p id = "dupliName" class = "errorMessage"></p>
				     <p id = "dataError" class = "errorMessage"></p>
                 <form action = "update_language_insert.php" method = "post">
                <div class="form-group row">
                  <div class="col-sm-8 mb-3 mb-sm-0">
				  <span style = "color:black;"><b>Last Update:</b></span>
				  <span style = "color:#e15d44;"><i><?php echo $upLangRow['last_update']; ?></i></span>
				  <br/>
				  <br/>
				     <span style = "color:black;"><b>Language name:</b></span>
                    <input type="text" class="form-control form-control-user" name="langUpName" minlength = "3" title = "Characters only and minimum length is 3" pattern="[^\s][A-Za-z\s]+" maxlength="20" required="required" id="langUpName" placeholder="" value = "<?php echo $upLangRow['name'];  ?>">
					<input type = "hidden" name = "lanID" id = "lanID" value="<?php echo $upLangRow['language_id']; ?>">
                  </div>
				 ...
				 <button  name = "submitUpLanguage"  type="submit" class="btn btn-primary btn-icon-split btn-sm"><span class="icon text-white-50">
                  ...
                  <span class = "text">Update language</span></button>
                 </form>
				 <br/>
				  <?php if (isset($langID)){ 
				  echo("<a  class='btn btn-danger btn-user' href='tables_language.php'>Back</a>");
				   } ?>
                </div>
              </div>
  ...
  <div class="modal fade" id="logoutModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
   ...
   <?php 
			     ...
			     if(isset($_POST['inside_languageUP'])){
					 echo ("<script>sameLan(1);</script>");
				 }
			     else{
					 
					 echo ("<script>sameLan(0);</script>");
				 }
				 if(isset($_POST['dupli_languageUP'])){
					  echo ("<script>dupli_langName(1);</script>");
				 }
				  else{
					 
					 echo ("<script>dupli_langName(0);</script>");
				 }
				 
				  if(isset($_POST['errorLanN'])){
