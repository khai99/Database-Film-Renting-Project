...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/selectize.js/0.12.6/css/selectize.bootstrap3.min.css" integrity="sha256-ze/OEYGcFbPRmvCnrSeKbRTtjG4vGLHXgOqsyLFTRjg=" crossorigin="anonymous" />
  ...
  <script type="text/Javascript">
     function sameValuePayment($value){
		 if($value == 1){
		 document.getElementById("sameValPay").innerHTML="Invalid update: same value as before";
		 }else{
			 
			 document.getElementById("sameValPay").innerHTML="";
		 }
		 
		 
	 }
   function unableUpdateP($value){
	   if($value == 1){
		 document.getElementById("noUpdateP").innerHTML="Unable to update the record";
		 }else{
			 
			 document.getElementById("noUpdateP").innerHTML="";
		 }
...
<body id="page-top">
  <?php 
			  ...
			  if(isset($_POST['editPayment'])){
				  $paymentIDD = $_POST['editPayment'];
				
			  }else if(isset($_POST['update_payment'])){
			      $paymentIDD = $_POST['update_payment'];
			  }
			  else{
			  ...
			  $updatePayment = "SELECT * FROM `payment` WHERE payment_id = '$paymentIDD'";
		      $resultR=mysqli_query($links,$updatePayment);
			  $editPayment=mysqli_fetch_assoc($resultR);	
			 if(empty($editPayment['payment_id'])){
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <!-- Page Heading -->
          <h1 class="h3 mb-1 text-gray-800">Update Payment ID :  <span style = "color:#e15d44;"><?php echo $paymentIDD;  ?></span></h1>
          ...
          <div class="row">
              ...
              <div class="card position-relative">
                <div class="card-header py-3">
				 <div class="text-center">
                  <h6 class="m-0 font-weight-bold text-primary">Enter the latest payment information below to be updated</h6>
				  </div>
                </div>
                <div class="card-body">
				<p id = "sameValPay" class = "errorMessage"></p>
					<p id = "noUpdateP" class = "errorMessage"></p>
					
				  <form action = "update_payment_insert.php" method = "post" >
				    <div class="form-group">
					<span style = "color:black;"><b>Last Update:</b></span>
                    <span style = "color:#e15d44;"><i><?php echo $editPayment['last_update']; ?></i></span>
				      
					  
                </div>
					<div class="form-group">
					<span style = "color:black;"><b>Customer Details: </b></span>
					<span style = "color:black;"><i><?php 
					$sqlGetC = "SELECT payment.customer_id, customer.email FROM `payment` INNER JOIN `customer` ON payment.customer_id = customer.customer_id WHERE payment_id = '$paymentIDD'";
					...
					echo $fetch_Cus['customer_id']; echo '-'; echo $fetch_Cus['email']; } ?></i></span>
                    
                </div>
					<div class="form-group">
					<span style = "color:black;"><b>Staff Details: </b></span>
					<?php 
						$selectStaff = "SELECT payment.staff_id,staff.first_name,staff.last_name FROM `payment` INNER JOIN `staff` ON payment.staff_id = staff.staff_id WHERE payment.payment_id = '$paymentIDD'";
						$runS = mysqli_query($links,$selectStaff);
						$result = mysqli_fetch_assoc($runS);
						if(!empty($result)){
					?>
					<span style = "color:black;"><i><?php echo $result['staff_id']; echo '-'; echo $result['first_name']; echo '&nbsp'; echo $result['last_name']; ?></i></span>
						<?php }else{ ?>
							<span style = "color:black;"><i><?php echo '-'; ?></i></span>
						<?php } ?>
                </div>
				<div class="form-group">
					<span style = "color:black;"><b>Rental ID: </b></span>
					<span style = "color:black;"><i><?php 
						if(empty($editPayment['rental_id'])){
							echo '-';
						}else{
					echo $editPayment['rental_id'];
						} ?></i></span>
                    
                </div>
					<div class="form-group">
					<span style = "color:black;"><b>Payment Amount: </b></span>
					<span style = "color:black;"><i><?php echo $editPayment['amount']; ?></i></span>
                    
                </div>
				  <div class="form-group">
					<span style = "color:black;"><b>Payment Date: </b></span>
					<span style = "color:black;"><i><?php 
						if(empty($editPayment['payment_date'])){
							echo '-';
						}else{
							echo $editPayment['payment_date'];
						} ?></i></span>
                    
                </div>
                 <div class="form-group">
					<span style = "color:black;"><b>Payment Status<span style = "color:#659EC7;"><i>  *Can be updated and 0 for unpaid and 1 for paid</i></span></b></span>
                     <select autocomplete = "off" required = "required" name = "up_paymentStatus" id="up_paymentStatus"  placeholder="" >
					
						 
						 <?php
						        $selectedP = ($editPayment['payment_status'] ==  1) ? 'selected' : ''; //paid
								 echo "<option value='1' $selectedP >1</option>";
			...
			<div class="form-group">
					<span style = "color:black;"><b>Payment Type: </b></span>
					<span style = "color:black;"><i><?php 
						$selectPayType = "SELECT payment.payment_type_id,payment_type.name FROM `payment` INNER JOIN `payment_type` ON payment.payment_type_id = payment_type.payment_type_id WHERE payment.payment_id = '$paymentIDD'";
						$running = mysqli_query($links,$selectPayType);
						$resultss = mysqli_fetch_assoc($running);
						if(empty($resultss['payment_type_id'])){
							echo '-';
						}else{
							echo $resultss['payment_type_id'];echo '-'; echo $resultss['name'];
						} ?></i></span>
                    
                </div>
				  
                    
				   <input type="hidden" id="up_paymentId" name="up_paymentId" value="<?php echo $editPayment['payment_id']; ?>">
				   
				<button Onclick="return ConfirmUpdatePayment();" type = "submit"  name = "submitPayment"  type="submit" class="btn btn-primary btn-user btn-block">Update Payment</button>
				
				  </form>
				  <br/>
				  <?php if (isset($paymentIDD)){ 
				  echo("<a  class='btn btn-danger btn-user' href='tables_payment.php'>Back</a>");
				   } ?>
              </div>
...
<?php
	 ...
	 if(isset($_POST['same_value_payment'])){
		 echo ("<script>sameValuePayment(1);</script>");
	 }
	 else{
					 
		 echo ("<script>sameValuePayment(0);</script>");
	 }
    
	if(isset($_POST['errorUpdateP'])){
		 echo ("<script>unableUpdateP(1);</script>");
	 }
	 else{
					 
		 echo ("<script>unableUpdateP(0);</script>");
	 }
