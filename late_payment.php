...
<link rel = "shortcut icon" href = "img/fav.png" />
  ...
  <script>
    function ConfirmCustomerPaidLate()
    {
      var x = confirm("Are you sure that Customer has Paid for the late rental");
      if (x)
          return true;
      else
        return false;
        ...
        <div class="container-fluid">
          ...
          <div class="card shadow mb-4">
            <div class="card-header py-3">
              <h6 class="m-0 font-weight-bold text-primary">Late Payment</h6>
            </div>
            <div class="card-body">
			<?php  
                ...
                <table class="table table-bordered" id="dataTable" width="90%" cellspacing="0">
                  ...
                  <tbody>
                    <?php 
					     ...
					     $sqlPayOn = "SELECT payment.payment_id,payment.customer_id,payment.rental_id,rental_status.status_name,payment.amount,payment.payment_date,payment_status.status_name,payment_type.name,payment.last_update FROM `payment` INNER JOIN `payment_type` ON payment.payment_type_id = payment_type.payment_type_id INNER JOIN `payment_status` ON payment.payment_status = payment_status.status INNER JOIN `rental` ON payment.rental_id = rental.rental_id INNER JOIN `rental_status` ON rental.status_id = rental_status.status_id WHERE payment.payment_type_id = '3' AND payment.payment_status = '0'";
						 $query2=mysqli_query($links,$sqlPayOn);
					    
						while($rowPL=mysqli_fetch_array($query2)){
					?>
					<tr>
					  <td><?php echo $rowPL['payment_id']; ?></td>
					  <td><?php

					  if(empty($rowPL['customer_id'])){
						  echo '-';
					  }else{
						  echo $rowPL['customer_id'];
					  } 
					  
					  
					  
					  ?></td>
					   <td><?php echo $rowPL['rental_id']; ?></td>
					    <td><?php echo $rowPL['status_name']; ?></td>
					    <td><?php echo $rowPL['amount']; ?></td>
					   <td><?php
					   if(empty($rowPL['payment_date'])){
					       echo '-';
						 
						 }else{
							 echo $rowPL['payment_date'];
						 } 
					   ?></td>
					   <td><?php echo $rowPL['status_name']; ?></td>
					   
					    <td><?php echo $rowPL['name']; ?></td>
					  <td><?php echo $rowPL['last_update']; ?></td>
					   <form action = "handle_late_payment.php" method = "post">
					  <td>
							<button Onclick="return ConfirmCustomerPaidLate();"  name = "paidLateButton" id = "paidLateButton" value = "<?php echo $rowPL['payment_id']; ?>" data-toggle="modal" class="btn btn-danger"><span class="glyphicon glyphicon-edit"></span>Pay</button> 
