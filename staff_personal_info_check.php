...
<?php
	...
	$sqlherealso = "SELECT staff.first_name, staff.password,staff.last_name FROM staff WHERE staff.email = '$email'";
	...
	else
	{
		...
		if($password!=$repeat_password)
		{
			...
			$colors = "red";
			echo("<form id = 'form_to' style = 'display:none;'action = 'staff_page.php'method = 'post'> <input value = '$return_this'type = 'text' name = 'return_message' /><input type = 'text' value = '$colors'name = 'colors' /></form>");
		...
		else if ($password=="")
		{
			...
			$sqlhere = "UPDATE `staff` SET `first_name` = '$first_name',`last_name` = '$last_name',`last_update` = current_timestamp() WHERE staff.email = '$email'";
			mysqli_query ($links,$sqlhere);
			echo("<form id = 'form_to' style = 'display:none;'action = 'staff_page.php'method = 'post'> <input value = '$return_this'type = 'text' name = 'return_message' /><input type = 'text' value = '$colors'name = 'colors' /></form>");
		}
		else
		{
			$sqlhere = "UPDATE `staff` SET `first_name` = '$first_name',`password` = '$password',`last_name` = '$last_name',`last_update` = current_timestamp() WHERE staff.email = '$email'";
			mysqli_query ($links,$sqlhere);
			echo("<form id = 'form_to' style = 'display:none;'action = 'staff_page.php'method = 'post'> <input value = '$return_this'type = 'text' name = 'return_message' /><input type = 'text' value = '$colors'name = 'colors' /></form>");
		}
