...
<link rel = "shortcut icon" href = "img/fav.png" />
  ...
  <style>
   ...
   <script>
     function sameCate($value){
		 
		 if($value == 1){
		 document.getElementById("errorCategory1").innerHTML="Invalid update : Same value found";
		 }else{
	...
	function dupli_cateName($value){
		 
		 if($value == 1){
		 document.getElementById("dupliCate").innerHTML="Invalid update : Duplicate value found";
		 }else{
	...
	function error_UpdateC($value){
		if($value == 1){
		 document.getElementById("dataErrorC").innerHTML="Invalid update : Cannot be updated to database";
		 }else{
			 
			 document.getElementById("dataErrorC").innerHTML="";
		 }
	}
...
<?php 
  if(isset($_POST['updateCate'])){
	  ...
	  $cateID = $_POST['updateCate'];
	  
	  
  }else if(isset($_POST['update_category'])){
	  $cateID = $_POST['update_category'];
   ...
   $sqlCategoryID = "SELECT * FROM `category` WHERE category_id = '$cateID'";
   $resultCateUP=mysqli_query($links,$sqlCategoryID);
   $upCateRow=mysqli_fetch_assoc($resultCateUP);
...
<li class="nav-item">
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <!-- Page Heading -->
          <h1 class="h3 mb-4 text-gray-800">Update the category details</h1>
            ...
            <div class="col-lg-6">
             ...
             <div class="card shadow mb-4">
                <div class="card-header py-3">
                  ...
                  <h6 class="m-0 font-weight-bold text-primary">Update Category : <span style = "color:#e15d44;"><?php echo $upCateRow['category_id']; ?></span></h6>
                </div>
                <div class="card-body">
				<p id = "errorCategory1" class = "errorMessage"></p>
					<p id = "dupliCate" class = "errorMessage"></p>
				     <p id = "dataErrorC" class = "errorMessage"></p>
                 <form action = "update_category_insert.php" method = "post">
                <div class="form-group row">
                  <div class="col-sm-8 mb-3 mb-sm-0">
				  <span style = "color:black;"><b>Last Update:</b></span>
				  <span style = "color:#e15d44;"><i><?php echo $upCateRow['last_update']; ?></i></span>
				  <br/>
				  <br/>
				     <span style = "color:black;"><b>Language name:</b></span>
                     <input type="text" class="form-control form-control-user" name="cateName" minlength = "3" title = "Characters only and minimum length is 3" pattern="[^\s][A-Za-z\s]+" maxlength="25" required="required" id="cateName" placeholder="Category Name" value= "<?php echo $upCateRow['name'];?>">
					<input type = "hidden" name = "cateID" id = "cateID" value="<?php echo $upCateRow['category_id']; ?>">
                  </div>
				 ...
				 <button  name = "submitUpLanguage"  type="submit" class="btn btn-primary btn-icon-split btn-sm"><span class="icon text-white-50">
                  ...
                  <span class = "text">Update category</span></button>
                 </form>
				 <br/>
				  <?php if (isset($cateID)){ 
				  echo("<a  class='btn btn-danger btn-user' href='tables_category.php'>Back</a>");
				   } ?>
                </div>
              </div>
  ...
  <div class="modal fade" id="logoutModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
   ...
   <?php 
			     ...
			     if(isset($_POST['inside_categoryUP'])){
					 echo ("<script>sameCate(1);</script>");
				 }
			     else{
					 
					 echo ("<script>sameCate(0);</script>");
				 }
				 if(isset($_POST['dupli_categoryUP'])){
					  echo ("<script>dupli_cateName(1);</script>");
				 }
				  else{
					 
					 echo ("<script>dupli_cateName(0);</script>");
				 }
				 
				  if(isset($_POST['errorCateN'])){
					  echo ("<script>error_UpdateC(1);</script>");
				 }
				  else{
					 
					 echo ("<script>error_UpdateC(0);</script>");
				 }
