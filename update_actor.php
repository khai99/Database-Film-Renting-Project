...
<link rel = "shortcut icon" href = "img/fav.png" />
  ...
  <style>
   ...
   <script>
     function sameActor($value){
		 
		 if($value == 1){
		 document.getElementById("errorActor1").innerHTML="Invalid update : Same value found";
		 }else{
			 
			 document.getElementById("errorActor1").innerHTML="";
		 }
		 
	 }
    
	function dupli_actorName($value){
		 
		 if($value == 1){
		 document.getElementById("dupliActor").innerHTML="Invalid update : Duplicate value found";
		 }else{
			 
			 document.getElementById("dupliActor").innerHTML="";
		 }
		 
	 }
	
	function error_UpdateA($value){
		if($value == 1){
		 document.getElementById("dataErrorA").innerHTML="Invalid update : Cannot be updated to database";
		 }else{
			 
			 document.getElementById("dataErrorA").innerHTML="";
		 }
	}
...
<?php 
  if(isset($_POST['updateActor'])){
	  ...
	  $actorsID = $_POST['updateActor'];
	  
	  
  }else if(isset($_POST['update_actor'])){
	  $actorsID = $_POST['update_actor'];
   ...
   $sqlActorID = "SELECT * FROM `actor` WHERE actor_id = '$actorsID'";
   $resultActorUP=mysqli_query($links,$sqlActorID);
   $upActorRow=mysqli_fetch_assoc($resultActorUP);
...
<li class="nav-item">
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <!-- Page Heading -->
          <h1 class="h3 mb-4 text-gray-800">Update the actor details</h1>
            ...
            <div class="col-lg-8">
             ...
             <div class="card shadow mb-4">
                <div class="card-header py-3">
                  ...
                  <h6 class="m-0 font-weight-bold text-primary">Update Actor : <span style = "color:#e15d44;"><?php echo $upActorRow['actor_id']; ?></span></h6>
                </div>
                <div class="card-body">
				<p id = "errorActor1" class = "errorMessage"></p>
					<p id = "dupliActor" class = "errorMessage"></p>
				     <p id = "dataErrorA" class = "errorMessage"></p>
					  <span style = "color:black;"><b>Last Update:</b></span>
				  <span style = "color:#e15d44;"><i><?php echo $upActorRow['last_update']; ?></i></span>
				  <br/>
                 <form action = "update_actor_insert.php" method = "post">
                <div class="form-group row">
                  <div class="col-sm-6 mb-3 mb-sm-0">
				 
				  
				     <span style = "color:black;"><b>First name:</b></span>
                  <input type="text" class="form-control form-control-user" name="fAName" minlength = "2" title = "Characters only and minimum length is 2" pattern="[^\s][A-Za-z\s]+" maxlength="45" required="required" id="fAName" placeholder="First Name" value = "<?php echo $upActorRow['first_name']; ?>">
					<input type = "hidden" name = "actorID" id = "actorID" value="<?php echo $upActorRow['actor_id']; ?>">
                  </div>
                     <div class="col-sm-6">
					  <span style = "color:black;"><b>Last name:</b></span>
                    <input type="text" class="form-control form-control-user" maxlength="45" minlength = "3" title = "Characters only and minimum length is 3"  pattern="[^\s][A-Za-z\s]+" name = "lAName"  id="lAName" required="required" placeholder="Last Name" value = "<?php echo $upActorRow['last_name']; ?>">
					
                  </div>
                </div>
				
				 <button  name = "submitUpLanguage"  type="submit" class="btn btn-primary btn-icon-split btn-sm"><span class="icon text-white-50">
                      <i class="fas fa-check"></i>
                    </span>
                  <span class = "text">Update actor</span></button>
                 </form>
				 <br/>
				  <?php if (isset($actorsID)){ 
				  echo("<a  class='btn btn-danger btn-user' href='tables_actor.php'>Back</a>");
				   } ?>
                </div>
              </div>
  ...
  <div class="modal fade" id="logoutModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
   ...
   <?php 
			     ...
			     if(isset($_POST['inside_actorUP'])){
					 echo ("<script>sameActor(1);</script>");
				 }
			     else{
					 
					 echo ("<script>sameActor(0);</script>");
				 }
				 if(isset($_POST['dupli_actorUP'])){
					  echo ("<script>dupli_actorName(1);</script>");
				 }
				  else{
					 
					 echo ("<script>dupli_actorName(0);</script>");
				 }
				 
				  if(isset($_POST['errorActorN'])){
					  echo ("<script>error_UpdateA(1);</script>");
				 }
				  else{
					 
					 echo ("<script>error_UpdateA(0);</script>");
				 }
