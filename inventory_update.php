...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/selectize.js/0.12.6/css/selectize.bootstrap3.min.css" integrity="sha256-ze/OEYGcFbPRmvCnrSeKbRTtjG4vGLHXgOqsyLFTRjg=" crossorigin="anonymous" />
  ...
  <style>
   ...
   <script>
     function sameValueI($value){
		 
		 if($value == 1){
		 document.getElementById("sameInven1").innerHTML="Did not update : same value found";
		 }else{
			 
			 document.getElementById("sameInven1").innerHTML="";
		 }
		 
	 }
    
	 function error_update($value){
		 
		 if($value == 1){
		 document.getElementById("errorupdateIn").innerHTML="Error in updating the data to database!";
		 }else{
			 
			 document.getElementById("errorupdateIn").innerHTML="";
		 }
...
$invenIDD = '';
if(isset($_POST['editInven'])){
	$invenIDD = $_POST['editInven'];

}else if(isset($_POST['update_inventory'])){
	$invenIDD = $_POST['update_inventory'];
   ...
   $sqlInvenID = "SELECT * FROM `inventory` INNER JOIN `inventory_status` ON inventory.inventory_status_id = inventory_status.inventory_status_id WHERE inventory_id = '$invenIDD'";
   $resultIUP=mysqli_query($links,$sqlInvenID);
   $upIRow=mysqli_fetch_assoc($resultIUP);
...
<li class="nav-item">
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <!-- Page Heading -->
          <h1 class="h3 mb-4 text-gray-800">Update new inventory details</h1>
            ...
            <div class="col-lg-9">
             ...
             <div class="card shadow mb-4">
                <div class="card-header py-3">
                  ...
                  <h6 class="m-0 font-weight-bold text-primary">Update new inventory ID : <span style = "color:#e15d44;"><?php echo $upIRow['inventory_id']; ?></span></h6>
				  <br/>
                </div>
                <div class="card-body">
				<p id = "sameInven1" class = "errorMessage"></p>
				<p id = "errorupdateIn" class = "errorMessage"></p>
                 <form action = "update_inventory_insert.php" method = "post">
				 <span style = "color:black;margin-left:1px;"><b>Last Update:</b></span>
				    <span style = "color:#e15d44;margin-left:10px;"><i><?php echo $upIRow['last_update']; ?></i></span>
					<br/>
                <div class="form-group row">
				 
                  <div class="col-sm-6 mb-3 mb-sm-0">
				   <br/>
				   <span style = "color:black;"><b>Choose a Film: </b></span>
                     <select autocomplete = "off" required = "required" name = "filmUP" id="filmUP"  placeholder="" >
					
						 
						 <?php
						   echo "<option value = ''>-- Select a film--</option>";
						   $sqli = "SELECT film_id,title FROM `film` WHERE 1";
						   $result2 = mysqli_query($links, $sqli);
						   
						   while($row = mysqli_fetch_assoc($result2)){
							   
							   $selected = ($upIRow['film_id'] ==  $row['film_id']) ? 'selected' : '';
							   
							   echo "<option value='$row[film_id]' $selected >$row[film_id]&nbsp;&nbsp;$row[title]</option>";
					...
					<input type = "hidden" name = "invenID" id = "invenID" value="<?php echo $upIRow['inventory_id']; ?>">
                  </div>
             				  <div class="col-sm-6 mb-3 mb-sm-0">
							  <br/>
				   <span style = "color:black;"><b>Choose a store: </b></span>
                     <select autocomplete = "off"  name = "storeUP"  id="storeUP" required = "required"  placeholder="" >
					     <?php
						   echo "<option value = ''>-- Select a store--</option>";
						   $sqli = "SELECT store_id FROM `store` WHERE 1";
						   $resultStore = mysqli_query($links, $sqli);
						   while($rowS = mysqli_fetch_assoc($resultStore)){
							   
							 $selectedS = ($upIRow['store_id'] ==  $rowS['store_id']) ? 'selected' : '';
							   
							   echo "<option value='$rowS[store_id]' $selectedS>$rowS[store_id]</option>";
				 ...
				 <div class="form-group row">
				 <div class="col-sm-6 mb-3 mb-sm-0">
				   <span style = "color:black;"><b>Choose a status: </b>
                     <select  autocomplete = "off"  name = "status_in" id="status_in" required="required" placeholder="" >
					
						 
						 <?php
						   echo "<option value = ''>-- Select a status --</option>";
						   $sql = "SELECT * FROM `inventory_status` WHERE 1";
						    $resulting = mysqli_query($links, $sql);
						   while($row = mysqli_fetch_assoc($resulting)){
							   
							 $selectedS = ($upIRow['inventory_status_id'] ==  $row['inventory_status_id']) ? 'selected' : '';
							   
							   echo "<option value='$row[inventory_status_id]' $selectedS>$row[inventory_status_id]&nbsp;&nbsp;$row[in_stat_name]</option>";
				 ...
				 <button  name = "submitTitle"  type="submit" class="btn btn-primary btn-icon-split btn-sm"><span class="icon text-white-50">
                      <i class="fas fa-check"></i>
                    </span>
                  <span class = "text">Update inventory</span></button>
                 </form>
				 <br/>
				  <?php if (isset($invenIDD)){ 
				  echo("<a  class='btn btn-danger btn-user' href='tables_inventory.php'>Back</a>");
				   } ?>
                </div>
              </div>
  ...
  <div class="modal fade" id="logoutModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
   ...
   <?php 
			     ...
			     if(isset($_POST['inside_InvenUp'])){
					 echo ("<script>sameValueI(1);</script>");
				 }
			     else{
					 
					 echo ("<script>sameValueI(0);</script>");
				 }
				  if(isset($_POST[' errorupdate'])){
					 echo ("<script>error_update(1);</script>");
				 }
			     else{
					 
					 echo ("<script>error_update(0);</script>");
				 }
