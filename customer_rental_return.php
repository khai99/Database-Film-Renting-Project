...
<link rel = "shortcut icon" href = "img/fav.png" />
  ...
  <script>
    function  ConfirmCustomerReturn()
    {
      var x = confirm("Are you sure that Customer Return the Film");
      if (x)
          return true;
      else
        return false;
        ...
        <div class="container-fluid">
          ...
          <div class="card shadow mb-4">
            <div class="card-header py-3">
              <h6 class="m-0 font-weight-bold text-primary">Store Return Process</h6>
            </div>
            <div class="card-body">
			<?php  
			   if(isset($_SESSION['cusReturn_message'])){ ?>
				   <div class="alert alert-<?=$_SESSION['msg_typeReturn']?>"> 
				   
			   <?php  
			     echo $_SESSION['cusReturn_message'];
				 unset($_SESSION['cusReturn_message']);          //<?= mean just show cut echo
                ...
                <table class="table table-bordered" id="dataTable" width="90%" cellspacing="0">
                  ...
                  <tbody>
                    <?php 
					     ...
					     $sqlRenReturn = "SELECT * FROM `rental` WHERE rental_date IS NOT NULL AND return_date IS NULL AND (status_id = '3' OR status_id = '4')";
						$queryReturn=mysqli_query($links,$sqlRenReturn);
					    
						while($rowR=mysqli_fetch_array($queryReturn)){
					?>
					<tr>
					  <td><?php echo $rowR['rental_id']; ?></td>
					  <td><?php

					  if(empty($rowR['rental_date'])){
						  echo '-';
					  }else{
						  echo $rowR['rental_date'];
					  } 
					  
					  
					  
					  ?></td>
					   <td><?php echo $rowR['inventory_id']; ?></td>
					    <td><?php echo $rowR['customer_id']; ?></td>
					   <td><?php
					   if(empty($rowR['return_date'])){
					       echo '-';
						 
						 }else{
							 echo $rowR['return_date'];
						 } 
					   ?></td>
					   <td><?php echo $rowR['staff_id']; ?></td>
					   
					   <td><?php echo $rowR['status_id']; ?></td>
					   
					 
					   <form action = "handle_customer_return.php" method = "post">
					  <td>
							<button Onclick="return ConfirmCustomerReturn();"  name = "returnButton" id = "returnButton" value = "<?php echo $rowR['rental_id']; ?>" data-toggle="modal" class="btn btn-danger"><span class="glyphicon glyphicon-edit"></span>Customer Return</button> 
