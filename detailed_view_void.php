...
<body id="page-top">
  <?php 
			  $paymentVoidID = '';
			  if(isset($_POST['viewVButton'])){
				  $paymentVoidID = $_POST['viewVButton'];
				
			  }
			  else{
				  echo("<script type='text/javascript'>
				alert('Error in fetching the data');
				window.location = 'view_void.php';
			    </script>");
				  
				  
			  }
			  
			  $viewPaymentRefund = "SELECT payment.payment_id,payment.staff_id,payment.rental_id,payment.amount,payment.payment_date,payment.payment_status,payment_status.status_name,staff.first_name,staff.last_name,payment.payment_type_id,payment_type.name,payment.last_update FROM `payment` INNER JOIN `payment_type` ON payment.payment_type_id = payment_type.payment_type_id INNER JOIN `payment_status` ON payment.payment_status = payment_status.status INNER JOIN `staff` ON payment.staff_id = staff.staff_id WHERE payment_id = '$paymentVoidID' AND payment_status = '3'";
		      $resultPR=mysqli_query($links,$viewPaymentRefund);
			  $rowPayV=mysqli_fetch_assoc($resultPR);	
			  if(empty($rowPayV['payment_id'])){
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <!-- Page Heading -->
          <h1 class="h3 mb-1 text-gray-800">View Payment ID :  <span style = "color:#e15d44;"><?php echo $paymentVoidID;  ?></span></h1>
          ...
          <div class="row">
              ...
              <div class="card position-relative">
                <div class="card-header py-3">
				 <div class="text-center">
                  <h6 class="m-0 font-weight-bold text-primary">View the latest void payment id details</h6>
				  </div>
                </div>
                <div class="card-body">
				<p id = "sameVal" class = "errorMessage"></p>
                    ...
                    <span style = "color:#e15d44;"><i><?php echo $rowPayV['last_update']; ?></i></span>
				...
				<div class="form-group">
					...
					$sqlGetC = "SELECT payment.customer_id, customer.email FROM `payment` INNER JOIN `customer` ON payment.customer_id = customer.customer_id WHERE payment_id = '$paymentVoidID'";
					...
					<span style = "color:black;"><i><?php echo $rowPayV['staff_id']; echo '-'; echo $rowPayV['first_name']; echo '&nbsp'; echo $rowPayV['last_name']; ?></i></span>
                    
                </div>
				<div class="form-group">
					<span style = "color:black;"><b>Rental ID: </b></span>
					<span style = "color:black;"><i><?php 
						if(empty($rowPayV['rental_id'])){
							echo '-';
						}else{
					echo $rowPayV['rental_id'];
						} ?></i></span>
                    
                </div>
					<div class="form-group">
					<span style = "color:black;"><b>Payment Amount: </b></span>
					<span style = "color:black;"><i><?php echo $rowPayV['amount']; ?></i></span>
                    
                </div>
				  <div class="form-group">
					<span style = "color:black;"><b>Payment Date: </b></span>
					<span style = "color:black;"><i><?php 
						if(empty($rowPayV['payment_date'])){
							echo '-';
						}else{
					echo $rowPayV['payment_date'];
						} ?></i></span>
                    
                </div>
                  <div class="form-group">
					<span style = "color:black;"><b>Payment Status: </b></span>
					<span style = "color:black;"><i><?php echo $rowPayV['payment_status']; echo '-'; echo $rowPayV['status_name']; ?></i></span>
			...
			<div class="form-group">
					...
					<span style = "color:black;"><i><?php 
						if(empty($rowPayV['payment_type_id'])){
							echo '-';
						}else{
							echo $rowPayV['payment_type_id'];echo '-'; echo $rowPayV['name'];
						} ?></i></span>
				  ...
				  <br/>
				  <?php if (isset($paymentVoidID)){ 
				  echo("<a  class='btn btn-danger btn-user' href='view_void.php'>Back</a>");
				   } ?>
              </div>
