...
<body id="page-top">
  <?php 
			  $paymentRefundID = '';
			  if(isset($_POST['viewRCButton'])){
				  $paymentRefundID = $_POST['viewRCButton'];
				
			  }
			  else{
				  echo("<script type='text/javascript'>
				alert('Error in fetching the data');
				window.location = 'view_refund.php';
			    </script>");
				  
				  
			  }
			  
			  $viewPaymentRefund = "SELECT payment.payment_id,payment.staff_id,payment.rental_id,payment.amount,payment.payment_date,payment_status.status_name,payment.payment_status,staff.first_name,staff.last_name,payment.payment_type_id,payment_type.name,payment.last_update FROM `payment` INNER JOIN `payment_type` ON payment.payment_type_id = payment_type.payment_type_id INNER JOIN `payment_status` ON payment.payment_status = payment_status.status INNER JOIN `staff` ON payment.staff_id = staff.staff_id WHERE payment_id = '$paymentRefundID' AND payment_status = '2'";
		      $resultPR=mysqli_query($links,$viewPaymentRefund);
			  $rowPayRe=mysqli_fetch_assoc($resultPR);	
			  if(empty($rowPayRe['payment_id'])){
				  
				    echo("<script type='text/javascript'>
				alert('Error in fetching the datass');
				window.location = 'view_refund.php';
			    </script>");
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <!-- Page Heading -->
          <h1 class="h3 mb-1 text-gray-800">View Payment ID :  <span style = "color:#e15d44;"><?php echo $paymentRefundID;  ?></span></h1>
          ...
          <div class="row">
              ...
              <div class="card position-relative">
                <div class="card-header py-3">
				 <div class="text-center">
                  <h6 class="m-0 font-weight-bold text-primary">View the latest refund payment id details</h6>
				  </div>
                </div>
                <div class="card-body">
				<p id = "sameVal" class = "errorMessage"></p>
                    ...
                    <span style = "color:#e15d44;"><i><?php echo $rowPayRe['last_update']; ?></i></span>
				...
				<div class="form-group">
					...
					$sqlGetC = "SELECT payment.customer_id, customer.email FROM `payment` INNER JOIN `customer` ON payment.customer_id = customer.customer_id WHERE payment_id = '$paymentRefundID'";
					...
					<span style = "color:black;"><i><?php echo $rowPayRe['staff_id']; echo '-'; echo $rowPayRe['first_name']; echo '&nbsp'; echo $rowPayRe['last_name']; ?></i></span>
                    
                </div>
				<div class="form-group">
					<span style = "color:black;"><b>Rental ID: </b></span>
					<span style = "color:black;"><i><?php 
						if(empty($rowPayRe['rental_id'])){
							echo '-';
						}else{
					echo $rowPayRe['rental_id'];
						} ?></i></span>
                    
                </div>
					<div class="form-group">
					<span style = "color:black;"><b>Payment Amount: </b></span>
					<span style = "color:black;"><i><?php echo $rowPayRe['amount']; ?></i></span>
                    
                </div>
				  <div class="form-group">
					<span style = "color:black;"><b>Payment Date: </b></span>
					<span style = "color:black;"><i><?php 
						if(empty($rowPayRe['payment_date'])){
							echo '-';
						}else{
					echo $rowPayRe['payment_date'];
						} ?></i></span>
                    
                </div>
                  <div class="form-group">
					<span style = "color:black;"><b>Payment Status: </b></span>
					<span style = "color:black;"><i><?php echo $rowPayRe['payment_status']; echo '-';echo $rowPayRe['status_name']; ?></i></span>
			...
			<div class="form-group">
					...
					<span style = "color:black;"><i><?php 
						if(empty($rowPayRe['payment_type_id'])){
							echo '-';
						}else{
							echo $rowPayRe['payment_type_id'];echo '-'; echo $rowPayRe['name'];
						} ?></i></span>
				  ...
				  <br/>
				  <?php if (isset($paymentRefundID)){ 
				  echo("<a  class='btn btn-danger btn-user' href='view_refund.php'>Back</a>");
				   } ?>
              </div>
