...
<link rel = "shortcut icon" href = "img/fav.png" />
  ...
  <script>
    function ConfirmCustomerReceiveds()
    {
      var x = confirm("Are you sure that Customer Received the Film");
      if (x)
          return true;
      else
        return false;
        ...
        <div class="container-fluid">
          ...
          <div class="card shadow mb-4">
            <div class="card-header py-3">
              <h6 class="m-0 font-weight-bold text-primary">Store Rental Process</h6>
            </div>
            <div class="card-body">
			<?php  
			   ...
			   <?php  
				 ...
				 unset($_SESSION['store_message']);          //<?= mean just show cut echo
              ...
              <div class="table-responsive">
                <table class="table table-bordered" id="dataTable" width="90%" cellspacing="0">
                  ...
                  <tbody>
                    <?php 
						 $staffEmails = $_SESSION['email'];
						 $sqli2 = "SELECT * from `staff` where email = '$staffEmails'";
						 $runS = mysqli_query($links,$sqli2);
						 $resultss = mysqli_fetch_assoc($runS);
						 $storeIDGet = $resultss['store_id'];   	
					     $sqlRenOn = "SELECT * FROM `rental` WHERE rental_date IS NULL AND status_id = '1' AND collect_store_id = '$storeIDGet'";
						$query2=mysqli_query($links,$sqlRenOn);
					...
					<tr>
					  ...
					  if(empty($rows['rental_date'])){
						  echo '-';
					  }else{
						  echo $rows['rental_date'];
					  } 
					  ...
					  ?></td>
					   ...
					   <td><?php echo $rows['status_id']; ?></td>
					   <td><?php echo $rows['collect_store_id']; ?></td>
					 
					   <form action = "handle_store_received.php" method = "post">
					  <td>
							<button Onclick="return ConfirmCustomerReceiveds();"  name = "receivedButton" id = "receivedButton" value = "<?php echo $rows['rental_id']; ?>" data-toggle="modal" class="btn btn-warning"><span class="glyphicon glyphicon-edit"></span>Customer Received</button> 
