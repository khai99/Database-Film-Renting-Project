...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/selectize.js/0.12.6/css/selectize.bootstrap3.min.css" integrity="sha256-ze/OEYGcFbPRmvCnrSeKbRTtjG4vGLHXgOqsyLFTRjg=" crossorigin="anonymous" />
  ...
  <style>
   ...
   <script>
	...
	function sameCityAsB($value){
		 
		 if($value == 1){
		 document.getElementById("samecity").innerHTML="Invalid update : Same value found as before";
		 }else{
			 
			 document.getElementById("samecity").innerHTML="";
		 }
		 
	 }
	
	function dupliFoundCity($value){
		if($value == 1){
		 document.getElementById("dupliCity").innerHTML="Invalid update : Duplicate value found";
		 }else{
			 
			 document.getElementById("dupliCity").innerHTML="";
		 }
	}
	
	function errorCity($value){
		if($value == 1){
		 document.getElementById("errCity").innerHTML="Invalid update : Unable to update the current city ";
		 }else{
...
<?php 
  if(isset($_POST['updateCity'])){
	  ...
	  $cityID = $_POST['updateCity'];
	  
	  
  }else if(isset($_POST['update_city'])){
	  $cityID = $_POST['update_city'];
   ...
   $sqlCityID = "SELECT * FROM `city` WHERE city_id = '$cityID'";
   $resultCityUP=mysqli_query($links,$sqlCityID);
   $upCityRow=mysqli_fetch_assoc($resultCityUP);
...
<li class="nav-item">
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <!-- Page Heading -->
          <h1 class="h3 mb-4 text-gray-800">Update the city details</h1>
            ...
            <div class="col-lg-7">
             ...
             <div class="card shadow mb-4">
                <div class="card-header py-3">
                  ...
                  <h6 class="m-0 font-weight-bold text-primary">Update City : <span style = "color:#e15d44;"><?php echo $upCityRow['city_id']; ?></span></h6>
                </div>
                <div class="card-body">
				<p id = "samecity" class = "errorMessage"></p>
						<p id = "dupliCity" class = "errorMessage"></p>
						<p id = "errCity" class = "errorMessage"></p>
                 <form action = "update_city_insert.php" method = "post">
                <div class="form-group">
               
				  <span style = "color:black;"><b>Last Update:</b></span>
				  <span style = "color:#e15d44;"><i><?php echo $upCityRow['last_update']; ?></i></span>
				  <br/>
				  <br/>
				     <span style = "color:black;"><b>City name: <span style = "color:#659EC7;"><i> <small> *Can be updated </small></i></span></b></span>
                     <input type="text" class="form-control form-control-user" name="cityNameUp" minlength = "2" title = "Characters only and minimum length is 2" pattern="[^\s][A-Za-z()\s]+" maxlength="50" required="required" id="cityNameUp" placeholder="City Name" value= "<?php echo $upCityRow['city'];?>">
					<input type = "hidden" name = "cityIDD" id = "cityIDD" value="<?php echo $upCityRow['city_id']; ?>">
                 
             
                </div>
				
				<div class="form-group">
				 <span style = "color:black;"><b>Country that associated with the city: <span style = "color:#659EC7;"><i> <small> *Can be updated </small></i></span></b></span>
                     <select autocomplete = "off" required = "required" name = "upCountryID" id="upCountryID"  placeholder="" >
					
						 
						 <?php
						   echo "<option value = ''>-- Select a country--</option>";
						   $sqli = "SELECT country_id,country FROM `country` WHERE 1";
						   $result2 = mysqli_query($links, $sqli);
						   
						   while($row = mysqli_fetch_assoc($result2)){
							   
							   $selected = ($upCityRow['country_id'] ==  $row['country_id']) ? 'selected' : '';
							   
							   echo "<option value='$row[country_id]' $selected >$row[country_id]&nbsp;&nbsp;&nbsp;&nbsp;$row[country]</option>";
				 ...
				 <button  name = "submitUpCountry"  type="submit" class="btn btn-primary btn-icon-split btn-sm"><span class="icon text-white-50">
                  ...
                  <span class = "text">Update city</span></button>
                 </form>
				 <br/>
				  <?php if (isset($cityID)){ 
				  echo("<a  class='btn btn-danger btn-user' href='tables_city.php'>Back</a>");
				   } ?>
                </div>
              </div>
  ...
  <div class="modal fade" id="logoutModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
   ...
   <?php 
				 ...
				 if(isset($_POST['same_city'])){
					  echo ("<script>sameCityAsB(1);</script>");
				 }
				  else{
					 
					 echo ("<script>sameCityAsB(0);</script>");
				 }
				 
				  if(isset($_POST['dupli_city'])){
					  echo ("<script>dupliFoundCity(1);</script>");
				 }
				  else{
					 
					 echo ("<script>dupliFoundCity(0);</script>");
				 }
				 
				 if(isset($_POST['error_city'])){
					  echo ("<script>errorCity(1);</script>");
				 }
				  else{
					 
					 echo ("<script>errorCity(0);</script>");
				 }
