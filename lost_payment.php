...
<link rel = "shortcut icon" href = "img/fav.png" />
  ...
  <script>
    function ConfirmCustomerPaidLost()
    {
      var x = confirm("Are you sure that Customer has Paid for the lost rental");
      if (x)
          return true;
      else
        return false;
        ...
        <div class="container-fluid">
          ...
          <div class="card shadow mb-4">
            <div class="card-header py-3">
              <h6 class="m-0 font-weight-bold text-primary">Lost Payment</h6>
            </div>
            <div class="card-body">
			<?php  
			   if(isset($_SESSION['payment_messageL'])){ ?>
				   <div class="alert alert-<?=$_SESSION['msg_typeL']?>"> 
				   
			   <?php  
			     echo $_SESSION['payment_messageL'];
				 unset($_SESSION['payment_messageL']);          //<?= mean just show cut echo
                ...
                <table class="table table-bordered" id="dataTable" width="90%" cellspacing="0">
                  ...
                  <tbody>
                    <?php 
					     ...
					     $sqlPayOn = "SELECT payment.payment_id,payment.customer_id,payment.rental_id,rental_status.status_name,payment.amount,payment.payment_date,payment_status.status_name,payment_type.name,payment.last_update FROM `payment` INNER JOIN `payment_type` ON payment.payment_type_id = payment_type.payment_type_id INNER JOIN `payment_status` ON payment.payment_status = payment_status.status INNER JOIN `rental` ON payment.rental_id = rental.rental_id INNER JOIN `rental_status` ON rental.status_id = rental_status.status_id WHERE payment.payment_type_id = '2' AND payment.payment_status = '0'";
						 $query2=mysqli_query($links,$sqlPayOn);
					    
						while($rowPLo=mysqli_fetch_array($query2)){
					?>
					<tr>
					  <td><?php echo $rowPLo['payment_id']; ?></td>
					  <td><?php

					  if(empty($rowPLo['customer_id'])){
						  echo '-';
					  }else{
						  echo $rowPLo['customer_id'];
					  } 
					  
					  
					  
					  ?></td>
					   <td><?php echo $rowPLo['rental_id']; ?></td>
					    <td><?php echo $rowPLo['status_name']; ?></td>
					    <td><?php echo $rowPLo['amount']; ?></td>
					   <td><?php
					   if(empty($rowPLo['payment_date'])){
					       echo '-';
						 
						 }else{
							 echo $rowPLo['payment_date'];
						 } 
					   ?></td>
					   <td><?php echo $rowPLo['status_name']; ?></td>
					   
					    <td><?php echo $rowPLo['name']; ?></td>
					    <td><?php echo $rowPLo['last_update']; ?></td>
					   <form action = "handle_lost_payment.php" method = "post">
					  <td>
							<button Onclick="return ConfirmCustomerPaidLost();"  name = "paidLostButton" id = "paidLostButton" value = "<?php echo $rowPLo['payment_id']; ?>" data-toggle="modal" class="btn btn-danger"><span class="glyphicon glyphicon-edit"></span>Pay</button> 
